...
**DEBUG** (CMake `COLONIZE_DEBUG_MENU`, shown when `debug.menu` is true): **Sprite Viewer** (same as `` ` ``) and **Show Mouse Coords** (toggles the pixel HUD attached to the pointer; on by default, persisted as `debug.mouse_coords`).

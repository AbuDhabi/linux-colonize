...
**DEBUG** (CMake `COLONIZE_DEBUG_MENU`, shown when `debug.menu` is true): **Sprite Viewer** (same as `` ` ``), **Show Mouse Coords** (pointer pixel HUD; off by default, persisted as `debug.mouse_coords`), **Building Rects** (colony-screen sprite bounds; off by default, persisted as `debug.building_rects`), and **Debug Logs** (`diag_info` to `colonize-linux.log`; off by default, persisted as `debug.logs`).
...
| Village trade intel (Linux-only) | `ICONS.SS` **#22–37** | Under the native camp line: `Buys:` row (goods named by @BRING / @BADCARGO / @CHIEFHOWDY) and `Sells:` row (@BUYWHICH), 13px rows, icons in one shared column; each row only once that dialog has been shown to the player. Runtime-only side table `village_trade_intel.c` keyed by settlement tile and European nation — never saved, wiped by `ai_contact_reset` (new game / Load) and on village removal. `render_map_panel … buys=a,b,c sells=a,b,c` seeds it |
...
**Orders / allegiance chrome** (`unit_chrome.c`, DOS `FUN_112b_01ba`): each unit graphic is a black silhouette shifted 2px left, then a nation-colored orders rectangle (`@ORDERS` letter), then the color sprite. Placement by Col1 type: foot units bottom-right; mounted / Caravel / Merchantman top-left; Galleon–Man-O-War top-right; Treasure / Artillery / Wagon top-center. A second under-rect marks stacks. England fill uses palette **112** (saturated red); NAMES `@COUNTRY` lists 12, which is pink `(255,85,85)` in VGA art palettes. The Dutch pair is `@COUNTRY` 13 / 5, which only `ICONS.SS`'s own palette defines as orange `(255,113,0)` / `(170,73,0)`; every other sheet leaves those slots as EGA magenta and paints no pixel with them, so the map screen merges ICONS' 5 and 13 into the TERRAIN-derived map palette at load (`game_loop.c`, ICONS.SS block) and screens with a foreign palette nearest-match the true RGB via `unit_chrome_*_for_palette` (bugs.md #364). Wired on the main map, sidebar, Europe docks/ships, colony Units + transport panes, and Colonizopedia unit previews.

Terrain move costs (plains 1, forest/hills 2, mountains 3; road/river halves) follow DOS `FUN_465b`: a unit may always enter when it still has its **full** movement allotment (remaining MP then exhausts). With **partial** MP into an over-budget tile, the game rolls `range(1, cost)` and succeeds if `roll ≤ remaining`; the full cost is charged either way (failed rolls leave the unit in place with 0 MP). Go-To pathfinding only commits guaranteed steps (no gambling) — so a Go-To / trade-route unit left with a third or two of MP in rough terrain has no legal step; it then **parks for the turn** (remaining MP dropped) instead of being handed back to the activation cycle, which used to hold the cursor on a piece that could never move (bugs.md #363).
...
Comprehensive GSOUND driver reverse-engineering, DOS BGM scheduler, and sound ID ranges: **See [assets_sound.md](assets_sound.md)**

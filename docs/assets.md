...
**DEBUG** (CMake `COLONIZE_DEBUG_MENU`, shown when `debug.menu` is true): **Sprite Viewer** (same as `` ` ``), **Show Mouse Coords** (pointer pixel HUD; on by default, persisted as `debug.mouse_coords`), and **Building Rects** (colony-screen sprite bounds; off by default, persisted as `debug.building_rects`).

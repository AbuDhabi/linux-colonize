...
| `unknown13_pad[4]` (+0xbe) → **writer located 2026-08-19, live DOSBox-X trace** | founding=0 confirmed; lategame non-zero now explained. Static search of the 3 decompiled exports found nothing because the writer isn't in them — live memory-watchpoint trace (played colony, DS:8542 colony-pointer anchor) caught the byte flip firing from a **periodic timer-tick interrupt handler** (segment `124C`, custom 32-bit tick counter at DS:0x8338/833A, alternating-tick dispatch), not from founding or any direct player action. Traced 3 call levels deep (`0E0E`→`1930`→`124C`) before stopping; exact store opcode not pinned, but confirms `save_format_map.md:168`'s `fortification_on_map[4]` reading and closes the "writer unlocated" gap. `visible_to_euro` (`+0xba`) sibling likely shares the same ISR-driven path, not separately re-checked |

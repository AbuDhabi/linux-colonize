...
- [x] **T1.8 (closed 2026-08-29) — `FUN_6662_0f74` pathfinding subsystem.** Confirmed clean but
  ...
  **2026-08-29 — row closed: the whole 0f74 subsystem re-read from the full decompiles (`viceroy_unpacked.c:104150-104660` for 0906/09ae/0b4e/0f74, `viceroy_overlays.c:86572-86900` for 0015bc, `:105010` for the 0007d8 populator) and the Linux port aligned line by line.** Fixes in `units.c`: (1) 0015bc window is |cand−goal| < 8 (goal−7..goal+7, the old 16-box admitted goal−8); popped cells above the unit's own cost are not expanded and the unit's tile never is (DS:0xa370 latch); (2) neighbour pick is `cost[cand] + owner term + edge(unit→cand)`, lower wins, equal only on strictly lower octile(goal,cand) — it was min cost[] with an octile tie; (3) the DS:0x1dd4 "+1" arm is the road/river pair rule (layer2 & 0xa on both tiles = road/colony pair, or minor river on both + cardinal step → edge 1; the populator sets 0x1dd4 = 1 so its floods are uniform BFS) — `units_flood_edge`; (4) a cross-domain candidate (land tile for a ship) is allowed only when it carries a settlement (`FUN_1000_8886`) *and* is the unit's own tile or the goal — DOS never routes through a colony; (5) the ownership terms are re-applied in the tail (`units_flood_owner_term`); (6) `FUN_6662_0906` is not a "first step" — it returns the 0015bc flood cost at the mover (DS:0xa370) or −1, so the populator gate `0 < cost < 8` = at most 6 steps, and 0009ae's neighbour validation is "sample within 8 and flood-reachable from the mover" (`units_coarse_reach`, now wired into `units_coarse_snap`); (7) dispatch: after a near-flood miss DOS still tries the coarse waypoint (flood only if 0b4e succeeded); in the far tier the flood always runs toward DS:0xa14e/0xa14c — the waypoint or the untouched goal when 0b4e returned early (Linux used to skip the flood entirely without a waypoint); a far-flood hit that reverses the last step while the unit already moved this turn falls to the scored fallback for Euro nations; Indian nations (> 3) never use the scored fallback (return −1). Regression test `unit_flood_river_pair_step` (test_units.c). `ctest` 48/48. **Golden note:** `golden_ai_turns` TURN4→5 unit 4 (Pioneer, goal (47,40)) now ends at (48,38) where the old code's (48,39) matched the save — but TURN5's `facing=4` (south) proves DOS walked (50,38)→(49,38)→(48,38)→(48,39), three cardinal minor-river steps at ⅓ MP each; the new tail picks that path, Linux just stops one step short because land MP is whole `@UNIT movement` units (a river step costs 1 of 1), not DOS thirds. That was an MP-model gap, filed and closed the same day as `port_plan.md` P5.8 (MP in thirds): with it, `golden_ai_turns` is byte-identical to the pre-pathfinding baseline and the pioneer lands on (48,39) by DOS's own path. Still not modelled, all documented at the wire sites: the sea "continent id == 1" main-ocean gate, the type ≥ 0x13 `FUN_1000_894e` tile gate (accessor unidentified), the DS:0xa370 cost-cap register (BX at 0015bc entry, convention unresolved — no Ghidra on this box), the `unit+0x314b != '9'` letter on the reversal check, and DOS reading DS:0xa572/0xa574 stale when its own snap failed (Linux skips that flood).

...
58. ai_contact.c:5385 — `static AiContactReparations s_reparations[4]` has no reset hook (siblings have `ai_goals_reset` etc.); a pending offer survives new-game/load and its resolve indexes `col1->tribe[]` of a different game. L **FIXED 2026-09-09** — `ai_contact_reset()` added (also clears the three per-tribe absolute-turn cooldown stamps, hoisted to file scope) and called from `ai_init_new_game`. NOTE: the LOAD path still calls no module reset at all — not `ai_goals_reset`, `founding_fathers_reset` or this one; that is a game_loop.c gap, out of this pass's scope.

...
49. ai_diplo.c:738-752 — invented −2 gold/turn "harassment" drain on any Euro at war with any tribe (human included), in a function whose other two arms are now empty stubs marked "no DOS counterpart". M **FIXED 2026-09-09** — deleted (whole arm). Evidence of absence: a sweep of all 103 gold-word (`n*0x13c + -0x77ce`) sites in the three decompiles finds no constant-valued treasury decrement anywhere in the game; every `*p = *p - X` takes a variable X with a matching credit.

50. ai_contact.c:1251-1252 — alarm floor of 80 after the +100 delta stomps the France/Pocahontas halving (ai_diplo.c:3746-3752, DOS 80844-80850); the sibling it claims to copy floors tribe friction, not `alarm_by_player`. M **FIXED 2026-09-09** — raw `alarm_by_player` floor deleted; the tribe-friction floor (what the sibling actually writes) stays. Test: France +100 now lands at 60.

51. ai_diplo.c:3236-3279 — `ai_diplo_military_score` is an invented blend (attack+defense, +3 naval, pop*2, +5 fort, gold/50, +(3−rank)*2) driving Euro war/peace in `ai_diplo_euro_balance`, while every sibling reads the real DS:0x941c mirror `stuff.land_combat_strength[]`; also double-counts power via rank. M **FIXED 2026-09-09** — score is now `stuff.land_combat_strength[]`. The band constants (10/15/20/30) were deliberately NOT rescaled: ×8 rescaling silences both RNG-gated arms and breaks golden_ai_turns TURN6→7.

52. ai_diplo.c:603 — `indian_hostility_sticky` stand-in written every turn into real DOS save byte nation+0x48, which col1_save.h:634-642 maps as the King's grace/waiver counter; ~10 ai_euro.c gates depend on it; on a DOS-authored save the first read is the wrong quantity. Same repurposing pattern prior-audit #72 retired. M **FIXED 2026-09-09** — stand-in moved to nation record +0x4b (`unknown26[11]`, DOS-dead byte); +0x48 renamed `king_grace_counter` and is now never written. All call sites use the named member, so ai_euro.c needed no change (two stale `unknown26[8]` comments there remain).

53. ai_contact.c:1710-1723 — `ai_contact_clamp_alarms` bands the mirror at 200 while both accessors and every DOS site clamp 0..100; raw readers (:2112 pair_friction seed, :7447 raid gate) see 101..200 while accessor paths see 100 — band tests in one file disagree about the same pair. M-L **FIXED 2026-09-09** — band is 0..100, matching both accessors and every DOS site.

54. ai_contact.c:2147 (also :2751, :2824) — dead disjunct: `pair_friction` already seeds from `alarm_by_player`, so `|| alarm_by_player >= 55` can never decide; gift gate's `friction>=55 || alarm>=55 || friction>=40` reduces to the last term. L **FIXED 2026-09-09** — reduced at all three sites with a comment.

55. ai.c:2673 — comment on fresh Brave spawn inverts the port's native MP polarity ("created spent, acts next turn" — 0 means fresh/acts now per turn.c:186). Behavior matches DOS; comment is a trap on the inversion axis. L **FIXED 2026-09-09** — comment rewritten; behaviour untouched.

56. ai_diplo.c:3482 — `uint16_t gold_before_upkeep` truncates uint32_t gold; 65536/131072 read 0, suppressing the war-upkeep status line. L **FIXED 2026-09-09** — snapshot is `uint32_t`; test covers a 65536 treasury.

57. ai_diplo.h:256/259 + ai_contact.c:628 + ai_diplo.c:501 — stale at-war band comments surviving prior-audit #79 (header still says relation < 50; live constant is 26). L **FIXED 2026-09-09** — comments now cite `AI_DIPLO_INDIAN_AT_WAR_REL` (26 / alarm > 0x4a).

58. ai_contact.c:5385 — `static AiContactReparations s_reparations[4]` has no reset hook (siblings have `ai_goals_reset` etc.); a pending offer survives new-game/load and its resolve indexes `col1->tribe[]` of a different game. L **FIXED 2026-09-09** — `ai_contact_reset()` added (also clears the three per-tribe absolute-turn cooldown stamps, hoisted to file scope) and called from `ai_init_new_game`. NOTE (closed 2026-09-09): the LOAD path called no module reset at all — not `ai_goals_reset`, `founding_fathers_reset` or this one. All three now run at the top of `game_apply_col1_save` (game_loop.c), the single funnel for every load (manual Load dialog and title-screen LOAD both reach it via `game_load_col1_slot`; there is no `--load` CLI). Safe there because `col1_bridge_apply` immediately rebuilds what the save owns — its tail calls `founding_fathers_sync_from_col1_after_load`, and FF bitmasks live in the save.
...
89. reports.c:3375-3381/:3422/:3540 — Indian chief-portrait comments contradict code and reports.md (say "always #113 pending golden"; code does 113+quartile, resolved in T5.3); leftover base-114 guard makes q==0 bypass the sprite-count bound. L-M **FIXED 2026-09-09** — the three comments (header block, `REPORTS_INDIAN_ICON_SPRITE`, `IndianRow.icon_sprite`) now state 113 + quartile and cite the T5.3 decode; the render guard floors at `REPORTS_INDIAN_ICON_SPRITE` and the sprite-count bound is also applied to the fallback. Render is unchanged for a loaded ICONS.SS (`ss_blit_sprite` already rejects out-of-range ids), so goldens are untouched.

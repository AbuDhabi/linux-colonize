...
| Fort: **+150%**; coastal fort slows / fires on adjacent enemy ships (wiki: attack strength 4 + 4 per artillery) | [Fort (Col)](https://civilization.fandom.com/wiki/Fort_(Col)) | Live land: same Stockade ladder (wiki +150% ≈ fortified); naval fire **Done** (`units_coastal_fort_fire_pulse`); the wiki's "slows the ship" half is **Done** (2026-09-16, `units_ship_slow_scan`): per-step `FUN_5bfb_3180` drains Fort +2 / Fortress dead-stop, no roll, plus the adjacent-warship roll; separate from fort fire, whose loss leaves MP alone (bugs.md #249) — [combat_analysis.md](combat_analysis.md#ship-slow-units_ship_slow_scan-2026-09-16) |

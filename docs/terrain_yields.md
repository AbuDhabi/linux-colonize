...
| `+0x3` | **Decoded 2026-08-21.** Values (idx 0–28): `2,2,4,4,4,4,2,2,3,1,3,3,3,3,1,1,3,1,3,3,3,3,1,1,0,3,0,2,2` — byte-identical across all 20 dump instances found (brute-force stride-match, no live capture). Confirmed 3 use sites, all inside `FUN_521d_20e6` (`viceroy_unpacked.c:85807/88865/91539`, three near-duplicate copies of the same block — an accumulating colony-site desirability score over a candidate site's 8 neighbor tiles, adding this byte for a neighbor once a coast/river-type gate (`local_6a`) passes; unforested farmland (Plains/Prairie/Grassland/Savannah) scores highest (4), Arctic/Sea-Lane 0, forest generally low (1–3). **Not worth wiring**: this whole `20e6`-inline candidate-tile picker is the same found/contact mechanism `ai_port_plan.md` **T1.2** already closed — DOS's own chain-linked-list bookkeeping it ultimately gates on is never actively maintained for land units in this port, and the *behavioral* outcome is already covered by shipped Linux mechanics (`ai_euro_unit_act`'s H-block bind, `ai_euro_scout_contact_ring_target`). Porting this byte into that path would be dead code, same conclusion as T1.2's case-2 gate. |
| `+0x4` | **Decoded 2026-08-21.** Values (idx 0–28): `12,12,10,15,15,21,12,14,18,12,18,12,12,12,12,7,18,12,18,12,12,12,12,7,0,9,9,24,24` — byte-identical across all 20 dump instances. Use site (`viceroy_unpacked.c:3629`, `*(byte*)(class*0x10+0x2f7a)`) is real — subtracted from a running byte (`pbStack_1e`, floor 1) near Indian-village globals (`0x8542`, `0x8dd2`), `+0x18` forest bonus gated on difficulty `<0xb`. **Enclosing-function attribution retracted, same session.** Its apparent home in the flattened export, `FUN_129f_0008`, is a **false collision** — a fresh Ghidra headless force-redecompile of that function's real resident address (`0000:29f8`, `GhidraForceRedecomp.java`, `OverlayTest` project) shows a completely unrelated RLE/byte-run graphics decoder, not this village-scoring code, and the flattened export's own `FUN_129f_0008` span carries a `WARNING: Could not emulate address calculation at 0x1eb83` / `Bad instruction - Truncating control flow` pair at line 3513-3525, *before* the `+0x4` use site at 3629 — the same "decompiler followed a bad jump and kept printing plausible-but-wrong content" signature already catalogued elsewhere in this project (`4528`'s `a6e4`, `T1.10`'s glyph-renderer collision). **The real function containing the `+0x4` read is unidentified** — needs proper jump-table-based resolution from a verified-clean entry point (`rtlink_overlay_extract.py` + fresh disassembly), not a guess from the flattened export's function boundaries. Deeper than `ai_port_plan.md` T1.14 assumed; re-scoped there as genuinely open RE, not a quick add. |

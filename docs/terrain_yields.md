...
| `+0x4` | **Decoded 2026-08-21.** Values (idx 0–28): `12,12,10,15,15,21,12,14,18,12,18,12,12,12,12,7,18,12,18,12,12,12,12,7,0,9,9,24,24` — byte-identical across all 20 dump instances. Use site (`viceroy_unpacked.c:3629`, `*(byte*)(class*0x10+0x2f7a)`) is real — subtracted from a running byte (`pbStack_1e`, floor 1) near Indian-village globals (`0x8542`, `0x8dd2`), `+0x18` forest bonus gated on difficulty `<0xb` — but its enclosing function is **`FUN_129f_0008`** (canonical resident `FUN_0000_29f8`), a large, currently **uncatalogued** function (not `152e`, not in `FUNCTION_CATALOG.md`/`address_mapping.csv` under any documented name, 5 params, several more unmapped globals: `-0x71a6`, `-0x7262`, `-0x6e74`/`-0x6e34`/`-0x6e80`/`-0x6e7c`). Porting this byte correctly needs its own dedicated RE pass on `FUN_129f_0008` first (same class of effort as `euro_g_table_0a60.md`'s multi-pass dig) — **not** the quick self-contained add `ai_port_plan.md` T1.14 assumed; re-scoped there. |

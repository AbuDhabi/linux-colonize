...
| `+0x4` | Labor/travel penalty subtracted from a colonist's candidate work-plot score in **`FUN_15eb_28c8`** (work-plot auto-assign; ported, and live for human colonies too since bugs.md #562/#570). Values: `12,12,10,15,15,21,12,14,18,12,18,12,12,12,12,7,18,12,18,12,12,12,12,7,0,9,9,24,24`. **Traps:** the earlier `FUN_129f_0008` attribution was a false collision (that body is an RLE decoder); the real owner was found via the `.asm`'s own XREF (`caseD_10`, physically in the `15eb` overlay), not a guess. `DS:0x8dd2` there is gross Lumber production (`1 < colony+0xa4 + *0x8dd2`, `+0xa4` = the colony's Lumber stock), not a proximity flag (bugs.md #895). **Wired** (bugs.md #577 corrected the stale "Not wired"): the table lives in `src/core/map.c` as `k_map_dos_terr_labor_penalty[29]` behind `map_dos_terr_labor_penalty_byte()`, and `ai_euro_28c8_score_full` (src/core/ai_euro_colony_jobs.c) applies it with the `+0x18` forest surcharge — and, since bugs.md #570, only inside 28c8's food-emergency branch (`bVar2`), which is where raw 13031-13050 puts it; it is not AI-only and not a universal base-score term. |
...
| `+0xa` | Also `k_map_dos_terr_lumber_reward` (Pioneer clear/plow lumber reward): `FUN_479b_01a6` raw 76785 reads `class*0x10 + 0x2f80`, and `0x2f80 - 0x2f76 = +0xa` = the **Lumberjack** column (bugs.md #613 — an earlier note wrongly put it at `+0x8`, the Cotton column). |
...
| 27 | Mountains | Ore Miner (uninhabitable, moot; FUN_15eb_1f72 12553-12570 strict max over jobs 1..7 skipping 5: Ore 4 beats Silver 1 — bugs.md #895) |
| 28 | Hills | Ore Miner |

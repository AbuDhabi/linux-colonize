...
| Cargo / `turns_worked` / chain | — | `mapped` | **Writers traced 2026-09-03** — `holds_occupied` (+0x0c) IS the authoritative goods-slot count: `FUN_15eb_30b8` (load good) tops up matching slots `0..ho-1` then appends at slot `ho` and INCs it, capacity-gated by @UNIT cargo column (`type*0xe+0x5237`); `FUN_15eb_317c` (remove slot) shifts higher slots down and DECs `ho` **without clearing the old top slot** — the source of the adjacent-duplicate stale pattern (`Tools:100,Tools:100`) seen everywhere; several unload/sell paths zero `ho` alone. So every byte at slot ≥ ho is stale garbage and the port's phantom-cargo gate is exactly right — no live DOS check needed. Spawn (`FUN_1427_06b4`) clears only holds[0..1], writes the 255 sentinel to hold[2] (Euro), and for **natives writes the current turn word (DS:`0x538e`) into holds[2..3]** — the last-visit stamp `FUN_4d56_021a` reads/writes (unit +0x12 word; port `col1_hold_raw[6..7]`) (game-start braves carry 0; the observed 64-107 values = spawn turns of regen braves); holds[3..5] otherwise inherit the dead previous unit's bytes (slot reuse), pioneers get hold[5]=100 tools. `FUN_479b_0158` (improvement done) does `hold[5] -= 20` with an unsigned `< 20` clamp check AFTER the subtract — calling it on a 0-tools unit underflows to 236/216/196, the mystery hold[5] family. **`cargo_hold[4]` (+0x14) on Wagon Trains (type 0xc) = the village-errand latch `+0x3158`** — Wagon-only AI scratch (every DOS reader gates on type 0xc; spawn clears it only for 0xc; 72-save census: 0/1 only), set by the 20e6 load matrix land arm, cleared by `FUN_4d56_2820` at trade entry; port round-trips it via `ai_euro_wagon_errand_get/_set` in `col1_bridge` (2026-09-07). Euro AI (`521d` 20e6) reuses holds[0..2] on AI units: hold[0] INC-capped-0x7f counter, hold[1] countdown, hold[2] wander-destination index 0..19 re-rolled `RNG(1,20)-1` when negative (the 255 sentinel doubles as "no destination"). Port preserves the raw +0x0c..+0x15 bytes per loaded unit (`col1_hold_raw` stash) and only overlays modeled goods/tools || `profession` | 1 | `mapped` | Job id; **Treasure** (`type==0x0a`): smcol + DOS Europe exit — value is gold/100 (`profession*100` cash; `FUN_48d3_06ba`) |
...
| `liberty_bells_total` / `liberty_bells_last_turn` | 2+2 | `mapped` | `total` = cumulative bells. `last_turn` = live EOT accrual; conditional stash on Linux save write when FF side table active, marked via `unknown21_pad` (see below) so a later load can tell a stashed pool apart from a genuine untouched value. All fixtures (starters + lategame + `TURN`) codec byte-identical — W1.5 (2026-08-24) found the "lategame/TURN codec drift" this row used to cite was fixed same-day it was written, by `753662d` "Fix FF + I work" (2026-08-22), which stashes/restores `unknown21_pad` alongside `liberty_bells_last_turn`; only the doc note calling it "drift" survived. See [savegame.md](savegame.md) Phase 5 |
...
| 156 | 16 | `0x95f2` | `continent_presence_flags[16]` — AI readers (`4d56`/`5952`); `= −0x6a0e`, **confirmed writer** `FUN_4962_0018` (2026-08-14): per-continent bitmask, called once per nation and **zeroed at the top of every call** (raw 78149-78150 clears `−0x6a0e[0..15]` before the scans, and `param_1` is the nation), so the byte always describes the nation currently being censused — an earlier "accumulates across the full per-turn pass" reading was refuted 2026-09-10 (audit C7): `1` = any Indian tribe/village exists on this continent (unconditional, no nation filter); `2` = a foreign unit of a nation whose owner nibble is < 4 — no domain filter, so a foreign ship on a land tile (docked in a colony) counts; a unit in a hold does not, because DOS parks passengers at the (−2,−2) sentinel where the tile lookup returns −1; `4` = a foreign (not this nation's) colony is present; `8` = this nation's own combat-capable land unit is caught out in the open (not on a settlement tile) with a specific pending-orders state. Matches `move_scoring_ship.md`'s already-flagged `−0x6a0e` trio member |
...
| 556 | 8 | `0x944e` | `avg_colony_pop[4]` (u16) — **resolved 2026-08-31 (static, raw `.asm`)**. `FUN_4962_0018` ADDs each owned colony's population (colony `+0x1f`) here during the per-nation sweep, then at the end of that sweep does `if (colony_counts[n] != 0) [0x944e+2n] /= colony_counts[n]` — so what is stored is the **mean colony size**, not a running total. Sole reader: `FUN_3f41_2548`'s Jan de Witt detail grid ("Average Colony", @MISC #97) — see [reports.md](reports.md) F8. Was `unknown_ds_944e`; JSON key string kept stable |
...
| `mask` | `mapped` | Occupancy + density rebuilt on export (`col1_bridge_sync_map_*`; `FUN_684c_08c0` / `137f_015e`); purchased sticky / layer2. Smcol/nawagers: `suppress` clears exhausted / far-ocean primes (not LCR). Nawagers: forest prime pattern is land pattern shifted **+4 columns** (clearing forest can reveal a different prime). Road is bit `0x08` only — bit `0x04` is the prime-**suppress** bit (`ColonizeCol1Mask.suppress` → `MAP_LAYER2_SUPPRESS`), unrelated; village/capital occupancy is bit `0x02` (`has_city`). (This row and two `col1_bridge.c` comments called `0x04` "village/capital occupancy" until 2026-09-09 — smell audit #77, prose only: struct, writer, constant and the layer2 derivation have always agreed on suppress.) **2026-08-25 fix:** the `map->improve[]` road-flag import checked `mask & 0x0c` (`0x04|0x08` combined) instead of `mask & 0x08` alone, so any tile with the suppress bit set — including plain ocean — got falsely flagged as roaded. Player-reported on `dutch-reports.SAV`: (47,31)/(53,32)/(55,34)/(55,37), all `mask=0x04`, all ocean, all wrongly roaded on load. `col1_bridge_apply` fixed to `mask & 0x08`. |
| `path` | `mapped` | Region + visitor. Nawagers: low nibble = path region (oceans/continents numbered independently; >15 → `0xF`); high nibble = last visitor (`0xF` unvisited; LCR cleared only on occupy, not 8-neighbor reveal). **2026-08-26 fix:** rumour presence is purely procedural (`map_procedural_rumour_at`, a position+seed hash) with no dedicated "already explored" bit anywhere in the Col1 tile/mask format — `map_clear_rumour` only ever set our own runtime-only `layer2` bit (`MAP_LAYER2_RUMOUR_CLEARED`), which starts zero on every fresh import, so a save carrying an already-resolved LCR showed it as freshly unexplored again. Player-reported on `dutch-reports.SAV` (46 stale tiles). Fixed by seeding that bit at import time from this very field's visitor nibble (`!= 0xF` → mark cleared) — resolving a rumour always means a unit physically stood on it, so "has anyone ever visited" already implies "any rumour here was already triggered"; harmless to set on a non-rumour tile too since `map_has_rumour_at`'s hash check still gates everything downstream. Also broadened `units_resolve_lcr_rumour`'s trigger from Scout-only (this port's own over-restriction) to any land unit, matching DOS — Scout / Seasoned Scout now roll from a better-weighted outcome table instead of being the only unit types able to trigger an LCR at all. |
...
| 612 | 2 | ? | (not the seed) | `open` | was labelled `prime_resource_seed`; 2026-09-15: value changes across the seed-100 TURN series (6, 6, 0xffff, 0…) while DS:0x190 = post_map `prime_resource_seed` (365 on seed 100, confirmed by FUN_137f_0598's rumour skip in `FUN_4d56_021a`) |

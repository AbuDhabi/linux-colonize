...
**2026-08-28, latest — presence bits DOS-exact + DOS-shaped dest reject (`ai_tile_tribe_or_presence`) + 0a60 coarse-plane restamp: TURN1→2, 2→3, 3→4, 6→7 green; TURN4→5 red on n=10 (48,41)/n=9 (36,52), TURN5→6 on n=7 (43,52). See ai_port_plan.md T1.23.** Older: **2026-08-28, later — presence-bit lifecycle made DOS-exact (see ai_port_plan.md T1.23): TURN1→2, 2→3, 5→6, 6→7 green; TURN3→4 red on three Braves, TURN4→5 on two.** The earlier 2-diff state relied on stale layer2 presence bits. Older note: **2026-08-28: TURN1→2, 2→3, 4→5, 5→6, 6→7 green; TURN3→4 red on two Braves** — n=6 (38,20) should step NE to (39,19), n=7 (48,46) should step E to (49,46). Both are Euro-state leaks into quiet scoring now that Euro slots above the human act before the Indians (DOS order): a stale layer3 presence stamp rejecting a dest at (46,53) (shifts later n=7 rolls) and a far-probe `−2` for the Dutch ship at (43,16). See ai_port_plan.md T1.23.

...
| # | Father | PEDIA.TXT effect (Tier 1) | DOS FUN | Port symbol(s) | Test | Status |
|---|--------|---------------------------|---------|-----------------|------|--------|
| 5 | Ferdinand Magellan | "The movement allowance of all naval vessels is increased by one, and the time to sail from the west map edge to Europe is shortened considerably." | `FUN_4345_0342` apply (elect bump); ongoing `+1` folded into move refresh; `FUN_48d3_0002` (landfall/goto duration roll, docks>2 + RNG>89 → 2 turns) | Elect: `effect_magellan_sea_moves` (one-time `moves_left++` for live ships). Ongoing +1: `turn.c` `turn_refresh_moves_for_nation` (~145). Save round-trip: `col1_bridge.c` ~991 also re-derives `moves_left` from raw `moves` with the same +1 when reading a Col1 save mid-turn (a **third** site, not previously listed in port_plan.md's "turn.c only" note — see Open items below). Landfall duration: `turn.c` ~2059. West-edge sail-time shortcut itself: not found in decomp (PARK, per code comment). | `test_founding_fathers.c` ~592-622 (elect permanent bump + refresh persistence) | **Done** for +1 movement (3 real sites, correcting the port_plan.md "turn.c only" claim — see Open items); west-edge sail-time shortcut **PARK** (no decomp evidence found) |
...
| 20 | William Brewster | "No more criminals or servants appear on the docks, and you select which immigrant in the Recruitment Pool will move to the docks." | `FUN_4345_0342` apply (gate) | `effect_brewster_filter_pool` (human-only, `europe.c`): refills/clears Criminal/Indentured/Servant from pool + dock, applied at elect **and** re-runnable (called only at elect, not ticked). "Player picks which immigrant moves to docks" (pick-among-pool UI) explicitly **PARK** per code comment — half the PEDIA text unported. | `test_founding_fathers.c` ~243-251 | **Thin** — no-criminals half **Done**; pick-among-pool UI half **PARK** (tracked, not silently dropped) |
...
5. **Brewster's "player selects which pool immigrant moves to the docks"**
   ...
   live. Already tracked (P4/P9.2), just confirming it's not silently
   dropped.

...
> **STATUS: historical audit dump, fully triaged/closed 2026-09-10 through
> the seventh fix wave (2026-09-15); not an authoritative spec; do not treat
> findings as facts.**
...
9. units.c — residual raw `moves` writes/reads that the `units_mp_charge/exhaust/restore` centralisation (prior-audit #6) did not reach: writes at :7499 (transport docking), :7795 (trade route), :7873/:7895 (pillage), :9742 ("FUN_281f_0934 stand-in: exhaust MP"), :10240/:10269 (board), and a raw read at :7843 (`u->moves <= 0` in `units_pillage`). All are Euro-only today (pillage is reached only from game_loop.c:11396, wagons/ships/pioneers are never native), so this is latent, not live — but they are the same inversion class and the helpers exist two screens away. L
...
- turn.c:148 `u->mp_spent_turn = 0` (bugs.md #423): citation checked against
...
1. **col1_bridge.c:2355-2360 — the `moves_spent` refund that smell #75 removed for land
   ...
   **104 of 254** on-map Euro transports with non-goto orders carry a nonzero spent byte, and
   ...
   partial-spend branch exports `max_mp - moves`. Left alone deliberately: the Europe
...
7. Equip-tools rounding: DOS `FUN_15eb_35d0` takes `min(stock, 100, req)` tools
   ...
   **REFUTED 2026-09-10 — wrong DOS function; the port is DOS-literal.**
   ...
   67774). So bugs.md #356's "a Pioneer legitimately walks with
...
2. `FUN_43f7_2244` (peacetime AI twin of the merc hire, raw 75100-75147) fills the
   ...
   **RESOLVED 2026-09-10 — 2244 now tails into the shared 10f0 paid arm; the third spawner is deleted.**
   ...
   map, so the `(hx, hy+1)` water-spawn class (bugs.md #255) is gone from
   ai_king.c entirely.
...
1. `@WARN%d` has **no latch at all** in DOS: raw 58538-58551 rebuilds and shows the
   ...
   beat with nothing but the `0x5382` bit test in front of it), and `market_demand_pool_raw[6]`

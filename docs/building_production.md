...
| — | **2026-08-13 check: the "messy" flag is real, not doc pessimism.** Re-disassembled via the overlay project — Ghidra's own function boundary for `1d4c` (497 bytes) is stale; the decompiler's control-flow walk shows the true function runs to roughly 19KB and hits a genuine overlapping-instruction warning around resident offset `0xbe05`. The opening slice (percentage-scaling formula reading a per-class byte at `unit+0x1a` against a 0x34-stride table at `DS:0x543f`) matches this doc's domain and looks legitimate, but it's part of a much bigger, partially corrupted function — not safe to re-peel from yet. Needs a dedicated `4528`-style boundary/offset recovery pass, not attempted this session; see `docs/decomp_inventory.md` |

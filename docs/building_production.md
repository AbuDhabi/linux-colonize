...
| **Not yet fixed, hammers/crosses:** same issue, more tangled | Investigated restructuring these the same way and found a real wrinkle: Carpenter/Preacher's DOS bodies use `base = skilled ? flat_6 : class_tag`, then `+= sol_bonus`, **then** double on a *colony-wide* "owns the upgraded building" flag (`FUN_15eb_038e`) — not the per-worker skill-match flag Statesman uses. The port's `church_rate=3`/`cathedral_rate=6` (and the equivalent house/shop tier lookup for hammers) bakes that colony-wide doubling into which rate constant gets used, which is algebraically equivalent to DOS *only when `sol_bonus` isn't folded in yet* — once it is, `scale_by_class(profession, 6) + sol` ≠ `(scale_by_class(profession, 3) + sol) × 2` (they differ by `sol` exactly), so a naive copy of the bells fix would be numerically wrong for the building-doubled case. Fixing this bit-exactly needs `colony_prod_crosses_worker`/`colony_prod_hammers_worker` to take the colony-wide building-owned flag as an explicit input (a signature/plumbing change touching `colony_prod_colony_crosses_ff`/`colony_prod_colony_hammers` and their callers), not just an extra `sol_bonus` parameter — real work, correctly scoped now, not attempted this pass. See deep peel Open questions. |

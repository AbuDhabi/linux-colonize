...
| **2026-08-15 fix (same bug, three more spots):** Tory penalty dropped for hammers/bells/crosses/field-yield preview | The same `sol_b > 0` guard (drop every Tory penalty, only apply SoL bonuses) was also in `turn.c`'s hammers block and `turn_count_bells_and_crosses_for_nation` (bells/crosses), and in `colony_preview.c`'s mirrors of all three plus its field-yield loop (that one wasn't even guarded the same way as `turn.c`'s already-correct unconditional field-yield add — a preview-only regression from copying the pattern). All five fixed: `sol_b != 0` guard, signed multiply/add, then clamp the colony-level total to ≥0 (DOS clamps *per worker* before summing — documented approximation, see deep peel). **Not fixed:** these three (unlike manufacturing) still add `sol_bonus` *after* the worker-rate functions' internal skill ×2, so a skilled Statesman/Preacher/Carpenter's SoL contribution still isn't doubled the way DOS doubles it — same bug class as the manufacturing fix, smaller magnitude, needs a real restructure (see deep peel Open questions), not attempted this pass. Regression: `test_turn.c` "Tory penalty reduces hammers/bells" (turn.c *and* preview) + a field-yield preview check reusing the existing lumberjack fixture |

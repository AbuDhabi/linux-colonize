...
4. **Hammers** — carpenters consume **lumber** toward `building_in_production`, **1:1 with the SoL-adjusted hammer count** (a high-SoL carpenter banking 14 hammers eats 14 lumber, not his sol-free base) — user-verified on `hammers_lumber.SAV`, bugs.md #163. This turn's field lumber is available: `FUN_15eb_1f72` raw 12683 passes gross hammers as lumber demand to `FUN_15eb_0b96`; `FUN_15eb_0b52` raw 10122-10138 computes unmet demand after gross lumber and starting stock, and the hammer output's input map at DS:0x2a2[16] is lumber. The older changelog rows below that describe sol-free lumber consumption or leave same-turn use unresolved are historical and superseded by this trace (audit #896/#918).
...
| **3 — Factory** | 2 | `* Factory`, Textile Mill, Iron Works, Arsenal (**Adam Smith**) | **9** | **6** (1.5× — `input = (total_out * 2) / 3`, one floor divide over the colony total; **except the muskets chain**, see below) |
...
| **Arsenal** | Factory-tier muskets **output** (9 per free colonist), but the tools **input** is 1:1 — **9 out / 9 in** (bugs.md #910). The muskets pair is the only chain DOS emits through `FUN_15eb_0b96` instead of `FUN_15eb_0bd4`: `FUN_15eb_1f72` raw 12689 closes the ledger with `FUN_15eb_0b96(0xe, gross[muskets] @DS:0x8de6)`, and both the factory `(G<<1)/3` discount (raw 10171) and the `(U*3)/2` unmet rescale (raw 10175-10180) live only inside `0bd4`. There is no `0bd4(0xe, 0xf)` call anywhere. The earlier “6 tools → 9 muskets” reading here was a port-side inference from the manual’s “half the tools” prose, not decomp, and is **wrong**. Armory 3←3, Magazine 6←6, Arsenal 9←9. |
...
| **Town Hall** | Worker rate **3**/worker unskilled, Elder Statesman ×2 (`FUN_15eb_1d4c`). The colony-wide sequence is `FUN_15eb_1f72` raw 12603-12650, on the bells word DS:0x8dec, in this exact order: worker loop → **`+= 1` unconditional** (raw 12619 — a bare add with **no** Town Hall `038e(9)` test, unlike the crosses word just above it, which does call `038e(0x25)`/`038e(0x26)`; a colony with no Town Hall row still earns the passive bell) → Jefferson (`3960(nation,0x0f)`) `+= v>>1` → Paine (`3960(nation,0x11)`) `+= (tax*v)/100` → AI subsidy (`3960(nation,0x12)` + the AI/non-human table gate) `+= (pop+3)/5` → `+= byte[0xa892]` (cleared to 0 two statements earlier, a no-op) → Newspaper (`038e(0x14)`) `v <<= 1` **else** Printing Press (`038e(0x13)`) `+= v>>1`. bugs.md #926/#927. |
...
`@TRAININDENTURED` / `@TRAINPROFESSION`. Ported in full 2026-09-04 (bugs.md #380 — the
...
| **Production strip** | Sum of assigned workers’ output | `colony_prod_building_display_output_sol()` → `colony_prod_worker_building_output_ctx()` |
...
| Factory input 6→9 | **Resolved 2026-08-15, player-confirmed (Viceroy) — the discount is real, and it tracks the actual SoL-adjusted output, not the flat base rate.** Earlier RE hit a dead end (`FUN_15eb_1d4c` only computes *output*; a "paired good" table looked promising but turned out to be the K-advisory warning-message system, a red herring). Settled by play instead: Textile Mill, free colonist, +2 sentiment — 12 cloth/turn output, 8 cotton/turn consumed (23 produced, 15 surplus colony-wide). `colony_prod_tier_input_for_output(FACTORY, 12) = (12*6+8)/9 = 8` — exact match; the un-modified-base-output reading (`(9*6+8)/9=6`) the port used to force would've been wrong. Fixed: `colony_prod_manufacturing_input` takes `sol_bonus` and threads it into the output calc it derives input from, same as the output side already did. Regression: `test_turn.c` (Iron Works, free colonist, sol_bonus=2 → output 12/input 8; sol_bonus=0 → input still 6, confirming the base-rate half wasn't broken). **Consumption code located 2026-09-23 (bugs.md #851):** `FUN_15eb_0bd4` (raw 10159-10182) → `FUN_15eb_0b96` → `FUN_15eb_0b52` write the demand word (DS-relative `-0x71f6`), and `FUN_15eb_0b0c` (raw 10102-10110) nets `gross - demand - unmet`. The requirement itself is `local_6 = (gross_out << 1) / 3` when `FUN_15eb_039e(chain) > 2` (factory tier) and `gross_out` 1:1 below it — ONE truncating divide over the colony's summed gross for the finished good, not a per-worker round-up. Both player anchors above (12→8, 9→6) are multiples of 3 and so did not distinguish the two; the port's `(out*6+8)/9` per colonist over-charged on odd totals and is now `colony_prod_chain_input_for_total_output` applied once in `colony_craft.c`. **Corrected 2026-09-24 (bugs.md #910):** the factory discount applies to the five `0bd4` chains only (ore→tools, tobacco→cigars, cotton→cloth, furs→coats, sugar→rum). The tools→muskets chain is emitted by `FUN_15eb_0b96` directly with the raw muskets gross as the tools demand, so it is 1:1 at every tier and its shortfall is never rescaled — `colony_prod_chain_input_is_flat`. Also (#913) a factory gross of 1 demands `(1*2)/3 == 0` and is produced free; the port used to drop the row. |
...
| **2026-09-16 (bugs.md #466): Autumn hammers freeze REMOVED** | The Spring-only hammers gate (turn.c Phase A/L, colony_preview.c `hammers_frozen`, colony_screen.c white-potential row) rested on one real-DOS pair, `colony-prod-tests/COLONY00→01_no-transports` (Spring→Autumn 1680), where hammers stayed put in all 32 colonies. Re-audited: not one colony in that save has a carpenter staffed (occupation 13), so the unchanged hammers prove nothing; the dutch2 Autumn→Spring pair banks normally and FUN_364b_0688 Phase L has no season term. Hammers now bank and debit lumber 1:1 on every tick; the calendar season is display only (starve/food popup text aside). The rows above that mention the Autumn freeze are historical. |

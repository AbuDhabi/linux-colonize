...
| Digital SFX (`COLDIG.BIN`) | Partial | Event ids `0x40..0x5c` are pushed in **AX**, which the decompile often drops. Sample decode, queueing, and mixing are done (`sound.c`, `gsound_vm.c`). The native attack typed cue `0x50`/`0x51` and separate generic attack cue are wired; `0x4c` is a generic attack variant and `0x5c` plays when buying horses. Combat loss cues `0x43`/`0x49`/`0x40`, partial village loss `0x48`, and Indian Raid woodcut suppression are matched. Per-effect execution counts still need a live DOS trace; see [assets_sound.md](assets_sound.md#trigger-parity-audit-2026-09-24). The Retire coin-tier tune is wired. |

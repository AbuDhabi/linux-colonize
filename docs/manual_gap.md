...
| Pick Music + BGM | Done | `gsound_vm.c` (literal `GSOUND.COL` driver emulator) + `sound.c`, `pick_music.c`; Sound Options popup (Background/Event/Sound Effects) **Done**, `options_dialog.c`. Song-id table corrected 2026-08-27 (Pick Music *n* is not `0x20+n`) — see [assets_sound.md](assets_sound.md) |
| Situational "Military" BGM cue on combat | Done thin | `units_combat_music_sting` (`units.c`) — DOS-evidenced (segment `5fef`), see [assets_sound.md](assets_sound.md) |
| Digital SFX (`COLDIG.BIN`) | Done | Event ids `0x40..0x5c` are pushed in **AX**, which the decompile often drops. Sample decode, queueing, and mixing are done (`sound.c`, `gsound_vm.c`). The static DOS launch expressions and port contexts now have an exact per-ID ledger, including computed and normally unreachable branches; every ID is launched by both programs and the normalized context counts match. See [assets_sound.md](assets_sound.md#trigger-parity-audit-2026-09-24). |
...
| Board / unload passengers | Done | **O** / **U**; hold icons; land→ocean with room auto-boards (`BOARD`); a zero amount means unload the whole selected hold |
...
| Build units in a colony (Artillery / Wagon Train / ships) | Done | bugs.md #481, 2026-09-17: all **seven** DOS unit projects, raw codes 42–48 = `@UNIT` rows 11–17 (`FUN_15eb_32f8`). Caravel/Merchantman/Galleon/Privateer/Frigate need a **Shipyard** and nothing else (`FUN_15eb_3650`); Man-O-War is out of the decode range. Hammers/tools now derive from the `@UNIT` cost/tools columns via `FUN_15eb_33aa` (`cost×0x20` with a 0x28 floor, `tools×10`) instead of two hardcoded pairs. Completion spawns on the colony tile (`FUN_364b_0114`) — chart in [building_production.md](building_production.md). 2026-09-23 (bugs.md #817): the two items previously listed as open both ship. **Wagon Train cap / `@NOMOREWAGONS`** — `colonies_wagon_cap_reached` (`colony_build.c`, DOS-LITERAL `FUN_15eb_3650` raw 13736-13740: `colony_counts[n] <= unit_type_counts[n][12]`, i.e. wagons < colonies) gates both the build-list availability and the spawn arm in `turn_colony.c` (DOS-LITERAL `FUN_364b_0114` raw 56926-56933: `@NOMOREWAGONS` with %NUMBER0 = colony count, `+0x1c |= 0x80`, project and hammers left intact so it re-fires each turn; popup is human-colony only, see bugs.md #789). **AI unit projects** — `ai_euro_colony_jobs.c` `ai_euro_5952_set_unit_project` picks Wagon Train `0x0c` (2385 arm: no WAGON_TRAIN flag, year < 0x640, continent presence & 1, ring1 == 0, nearest-village alarm < 0x32) plus the `0x0b`/`0x0f`/`0x10`/`0x11` Artillery/Galleon/Privateer/Frigate arms. |
...
| Docks immigrants from crosses | Done | Crosses pull from 3-slot recruit pool; Brewster filter + pick-among-pool **Done**; pool slot is DOS `RNG(0,2)` (`FUN_281f_04d4`) not player choice; **R** Recruit UI Done. Atomic `5e52` phase-6 tax-audience/FF-gift tail still **PARKED** in `ai_king.c`/`founding_fathers.c` (not a Europe-screen gap) |
...
| Missions / convert / incite | Partial | Missions are established only at a village: the human @ACTIONS rows 3/4 (`thunk_FUN_1000_a5dc` → `ai_contact_establish_mission`, `a594` → `ai_contact_denounce_heresy` with DOS's real weighted roll, `@HERESY0/1`, `MISSION0-3`), and the AI through `FUN_4d56_4528`'s non-human unit-type switch case 3 (`ai_contact_ai_missionary_village`: incite case 7 → establish case 3 → denounce case 4). The invented AI adjacency convert pulse, its 50/50 heresy, crosses bump, alarm decay and the missionary flee arm were deleted 2026-09-22 (bugs.md #557). Converts reaching a colony are the separate `FUN_5bfb_022e` `@INDIANSCONVERT` arm. HELLO1/2 greet **Done** thin; raid surprise/war chrome **Done** thin; incite/WARPATH gold **Done** for both arms (`FUN_4d56_417e` → `ai_contact_apply_incite`, 6th village-meet CHOICE; AI Mode 2 `ai_contact_ai_incite_human`, `port_plan.md` T4.5; [`indian_incite_417e.md`](../original_sources_annotated/ai/indian_incite_417e.md)) |
...
| Manual feature | Status | Notes |
|----------------|--------|-------|
| FF election from liberty bells | Done | Side-table bell pool (DOS +0xc); peacetime threshold elect + WoI pool→intervention (**Done** 2026-08-22); century-weighted debate pick (`4345_06d2`/`015a` **Done**); all 25 Fathers wired (2026-08-28); KINGGALLEON2 **Done** 2026-08-27 (`FUN_5fef_1908`); F3 portrait grid **Done**; FF pool bridge load **Done** |
| Pedia / F3 Congress report | Done | F3 page 1+2 match goldens (2026-08-25); debating highlight; leftover polish: fatter-than-DOS bell glyph, not a missing plate — [reports.md](reports.md) |
...
Whole-project **phase priority** lives in [port_plan.md](port_plan.md). This list
is the historical bring-up order (early manual chapters first), then the
**unparked queue** in [port_plan.md](port_plan.md) (prereqs met):
...
11. **Deep PARKED bodies** (full `2820` haggle, `4528` VGA meet chrome, FA `3f41`, T3 goldens) — signing cinematic **Done** 2026-08-30; HoF / end sequences **Done** thin — [port_plan.md](port_plan.md)

---

## Takeaway

Playability tracks P1–P11 closed 2026-09-03. The port is strong on **shell, map
art, navigation, reports / pedia, save, units / naval passengers, founding a
colony, Europe buy/sell/recruit/hire/equip, rumours/treasure (KINGGALLEON2
**Done**), Indian contact (`4528` human `@ACTIONS` **Done**; `2820` haggle
PARK), king/REF, FF election (all 25 Fathers), and trade routes**. Euro AI is
past structural (`20e6` / `5d04` **Done** 2026-08-27; deep −0x6790 remains).
Next work is `bugs.md` nits, production / combat depth on the Partial rows
above, VGA dialog chrome, and 1:1 AI bodies — not leftover KINGGALLEON2 or
unwired reports.

## See also

- [port_plan.md](port_plan.md) — whole-project phases / what’s next
...
- [port_plan.md](port_plan.md) — AI FUN_* inventory and 1:1 roadmap

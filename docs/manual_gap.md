...
| Digital SFX (`COLDIG.BIN`) | Done | Event ids `0x40..0x5c` are pushed in **AX**, which the decompile often drops. Sample decode, queueing, and mixing are done (`sound.c`, `gsound_vm.c`); combat, raids, movement, settlement, tax, chief-kill `0x55`, and the `0x8020`/`0x8024` chord contexts are mapped in [assets_sound.md](assets_sound.md#trigger-parity-audit-2026-09-24). The typed combat rule can select `0x50`/`0x51` for a player-controlled captured native attacker; `0x4c`/`0x5c` have no identified reachable push. The Retire coin-tier tune is wired. |

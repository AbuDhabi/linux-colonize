...
| `unknown05[2]` | **RESOLVED 2026-08-27 (static)**: bits 17-32 of the 32-bit once-only woodcut/splash "already shown" array at DS:`0x540a` (`event` = bits 1-16). Method that unstuck it: raw-`.asm` PUSH-immediate reads at all 13 `FUN_12fd_006c`/`FUN_281f_0524`/`051a` call sites. Literal ids in play: 1,2,3,5,6,7,8,9,10,11,13 (+3/4/5 computed in `FUN_5bfb`); `FUN_12fd_006c` itself: ids 1-10 via internal jump table, ids >10 forwarded to `FUN_6f30_0062(id)` = splash picture by index, negative id = show without setting the flag. Sole source of ids ≥14 is the demo-autoplay tail in `FUN_130d` (`viceroy_unpacked.asm:6912-6928`: `CMP [0x150],0x19; INC [0x150]; PUSH [0x150]; CALLF FUN_12fd_006c` loop, gated on DS:`0x828` demo flag and `turn%12==0`), so ids 17-25 = `unknown05[0]` bits 0-7 + `unknown05[1]` bit 0; bits 26-32 dead. Cleared by the new-game `memset(0x540a,0,4)`. Cross-check: the `FUN_13f1` call `PUSH 6` fires after `TEST AL,0x20` on a tile flag = `discovery_of_the_pacific_ocean` bit 6, matching the existing `event` naming. Header comment + `save_format_map.md` updated; field not renamed (`col1_json` key). |
| `unknown06_lo` (6 bits, player struct) → **confirmed dead 2026-08-27 (static)** | bits0-5 @ player+0x30 = DS:`0x543e` (player base `0x540e`, stride `0x34`). Exhaustive: every `0x543e` touch in all 3 exports + raw asm (`TEST/OR byte [BX+0x543e]`) uses only mask `0x40` (`lcr_case5_bonus_used`, `FUN_65dd_0004`) or `0x80` (`named_new_world`, `FUN_4720_049e`); no pointer-form access (`0x540e`-based refs are all `name[]` string uses). Bits 0-5 never read or written. Prior note: no reader/writer cite in either decompiled export. **Was 7 bits; bit6 resolved 2026-08-19** as `lcr_case5_bonus_used`, a per-nation one-shot in `FUN_65dd_0004` (the still-PARKED LCR/native-encounter result table, see `indian_contact.md`'s de Soto note) that upgrades a first-time case-5 roll to case 4. **Case 4/5 correspondence resolved 2026-08-24** (see catalog section B's new `FUN_65dd_0004` entry) — case 4 is the burial-mounds/"search for treasure" event (`@LOSTCITY4`+`@SCREWED`+`@BURIAL1`/`2`/`3`); case 5 is *not* an independently-displayed result at all, it's a transient internal roll bucket that always converts to case 4 (first LCR ever, this bonus) or case 6 ("nothing", no text) before the display dispatch runs — so the bit's own description ("upgrades case 5 to case 4") was already accurate, just case 5 never had a `@LOSTCITY5`/`@BURIAL` text of its own to begin with. |
| `unknown13_pad[4]` (+0xbe) → **RESOLVED 2026-08-27, static only** — renamed `fortification_on_map[4]` | Prior "no static writer" verdict was a grep artifact: DOS never addresses these bytes through the `*(int*)0x8542` colony pointer. It uses the colony **array** form `idx*0xca + 0x5d46 + off` (base derived from `flags` +0x1c = `0x5d62`), so +0xba → literal `0x5e00`, +0xbe → literal `0x5e04`. Grepping those literals finds everything: **writer `FUN_364b_1b4c(idx, nation)`** — `pop_on_map[nation] = population (0x5d65)`; `fort_on_map[nation] = FUN_15eb_03d6(idx, 0)` = count of owned buildings along the parent chain from building 0 (Stockade→Fort→Fortress ⇒ 0..3). Called (via thunk `FUN_281f_07c8`) from **tile-reveal `FUN_13f1_000a`** whenever a tile holding a colony is revealed to a nation; `FUN_13f1_000a` is called from fog-reveal-radius `FUN_13f1_0158` (unit sight, every move/turn). `FUN_13f1_00a6` (±5 reveal around a new colony) seeds `pop=1, fort=0` if pop was 0. Draw code (`FUN_112b_01ba` unit chrome, overlays `FUN_1000_9150/9d32/a05e/a07a/a0a4`) also sets `pop_on_map=1` if 0 for foreign colonies. **Reader** of `pop_on_map`: `FUN_364b_1b76` (foreign colony known-to-human gate, bypassed by `0x53a2 show_entire_map`). **No DOS reader of `fort_on_map` found** — colony sprite draw recounts live via `FUN_15eb_035e`; it is a write-only snapshot (likely vestigial, kept in save). The 2026-08-19 ISR/`DS:0xa660` lead is retracted: consistent with method note 3 (polling BPM reports foreground PC, not the writer) — the true write is the redraw/reveal path above. Old W4.4 closed. Catalog `FUN_364b_1b4c/1b76` labels "warehouse-capacity slots" are wrong — they are fog-of-war pop/fort snapshots. `visible_to_euro` name kept (7 live uses) but header comment now documents the real semantics |
...
| `unknown24_pad[4]` → **corrected 2026-08-27: write-only treasure-income accumulator** | `FUN_5fef_1908` (King's Galleon cash) does `nation+0x26 += net` (`-0x77d2`, 32-bit with carry); no reader. The 2026-08-19 "dead" verdict missed this negative-offset write. |
| `unknown26[12]` (nation +0x40..+0x4b) → **all 12 bytes now accounted for 2026-08-27** | Linux repurposed as diplo stand-ins (treaty_timer/diplo_flag/etc.) — the field split is a Linux invention, not a decode. DOS layout, complete: `+0x40-0x43` per-rival diplomatic cooldown (section D); `+0x44/+0x45` recruit RNG state; **`+0x46/+0x47` = int16 `last_colony_founded_turn`** (DS `-0x77b2`/`0x884e`): written `= DS:0x538e` (turn) by found-colony order body `FUN_479b_076e` (`viceroy_unpacked.c:77006`), read by unit desirability score `FUN_521d_052c` (`:87186`): `if founding_expansion_urgency(nation) != 0: score += (turn - stamp) >> 4` — the longer since the nation last founded, the more a founder-capable unit is worth; no other touch in any export or asm. `+0x48-0x4a` crosses/hammers carry accumulator; **`+0x4b` confirmed dead** (zero literal touches at `-0x77ad`/`0x8853`). Linux `diplo_flag[2..3]` sit on the turn stamp; harmless (DOS value not consumed by Linux AI), but not a decode. |
...
| `unknown34[12]` → **RESOLVED 2026-09-06d: leader trait triple** (the 2026-08-19 "confirmed dead" verdict is RETRACTED) | DS:0x9566. The 2026-08-19 sweep grepped only the positive `0x9566` spelling and so saw nothing but the bulk save-block R/W calls; every game-logic touch uses the negative literal `−0x6a9a`. Writer: the NAMES.TXT section loader's `@LEADERNAME` pass (`viceroy_unpacked.c:120960-120977`) — per nation, a name string into `DS:0x540e+n*0x34` then three signed numbers into `−0x6a9a[n*3+0..2]`. Shipped values `1,-1,0` / `0,1,0` / `1,0,-1` / `-1,0,1`; present in all 71 project `.SAV`s and in the DOSBox-X `Memory` dumps at `DS:237D+0x9566`. Readers: `FUN_5bfb_10ec` + `FUN_521d_153e` (col 0 = belligerence, both wired in Linux via `ai_diplo_leader_trait`), `FUN_521d_03d0` (col 1), `FUN_5952_035e` (cols 0/2, unported hosts). Field name kept — `unknown34_pad_hex` is a JSON key in checked-in fixtures. Full write-up: `original_sources_annotated/ai/euro_diplo.md` |
...
| `unknown46[32]` / `price_group_state[16]` → **formula traced 2026-08-19** | despite the name, not the displayed price — each of the 16 slots is a hidden per-cargo market **saturation/demand pool** (`FUN_38fd_0058`), seeded `RNG(600,1000)` at new game, topped up each call by summing a per-Euro-nation demand table. Index 0 (Food) gets an extra decay rule (turn1/nation0 only); indices 9-12 (Rum/Cigars/Cloth/Coats) get a floor-at-1 clamp — DOS *code* confirmation of the field's own long-standing "only these 4 form a live price group" smcol/experimental guess. `ColonizeCol1NationTrade.euro_price[16]` is the actual shown price. **Link traced 2026-08-27 (static, doc-sync — Linux already had it)**: same function's tail (`viceroy_unpacked.c:58830-58990`). For the two price groups (raw 1-4 with fur/idx 4 halved and +1/+2 pre-1700/pre-1600; manufactured 9-12) `target = 3*sum(group pools) / this pool` (`FUN_1d1d_0ec6` long div); EOT (`param_1==0`): `trade.nr[i] += sign(euro_price[i]-target) * ((rise_vol+fall_vol)/2)*100`; non-EOT: `euro_price[i] = clamp(target, min, max)`. Then per cargo: `nr += drift` (`0x96fe` table stride 9: `[0]`min `[1]`max `[2]`rise-volume `[3]`fall-volume `[4]`drift; Dutch doubles drift on odd turns), `nr <= -rise*100` → `nr += rise*100, price++` (cap = max, +`2*(diff-4)+(turn-600)/100` for idx>13 under AI), `nr >= fall*100` → `nr -= fall*100, price--` (floor min); human gets `@` price-change popups `0xfa8`/`0xfb0`. Linux: `europe_tick_market_prices` (`europe.c:1381+`) + `europe_apply_volume_price` — already implements the group-target and rise/fall arms. Still collides with the live king-stand-ins union on bytes 0-5 — not touched, same reasoning as the `unknown26` meta-mystery |
...
- **Nation-slot table ambiguity (resolved, but the resolution itself
  ...
  **Re-confirmed 2026-08-27 by struct math**: the "12 slots" are the
  bilateral relation row inside each Euro nation record — nation`+0x34`
  `euro_relation[4]` + `+0x38` `relation_by_indian[8]` = 12 contiguous
  peer bytes at DS `-0x77c4` (`FUN_15b3_0004`/`FUN_1000_8c28` read
  `nation*0x13c + peer - 0x77c4` for peer<4, Indian-side table at 23000
  for peer≥4). The *nation records themselves* are strictly 4 (`0x84fc`
  pointer, every site guards `3 < idx`; tribes live at
  `0x5ad6 + tribe*0x4e`). So: tribes have relation *slots* 4-11, never
  a `0x13c` record. Closed.
...
- `original_sources_annotated/ai/indian_settlement_4528.md` — ASM-faithful
  ...
  `0x1710`…`0x172e` **resolved 2026-08-27**: DS strings read straight
  from `VICEROY.EXE` (DS base = file `121248`, same calibration as the
  LCR `"LOSTCITY"` find): `0x1710`=`"VILLAGE"`, `0x1718`=`"WAR"`,
  `0x171c`=`"BAD"`, `0x1720`=`"MEDIUM"`, `0x1727`=`"SAVAGE"`,
  `0x172e`=`"HAPPY"` — base + band suffix = `GAME.TXT` tags
  `@VILLAGEWAR`/`@VILLAGEBAD`/`@VILLAGEMEDIUM`/`@VILLAGESAVAGE`/
  `@VILLAGEHAPPY` (lines 780/774/767/759/753). Same `strcpy`+append
  scheme as LCR; no index table.
- `original_sources_annotated/ai/indian_trade_2820.md` — human `CHOICE`
  dialog buy-offer path (`LAB_002e92`) — **resolved 2026-08-27 by
  reading the doc's own raw C (`:259-420`)**: there is no separate human
  price formula. `uStack_62` is computed once for both paths (base 200,
  or `(tribe_type-8)*-50` for cargo≥8; `+throttle[nation][cargo]*(2*diff+15)`
  for cargo≥7; `+RNG(0,base)`; `-4*unit_count_proxy`; `+4*relation`;
  `*0x8dc4/100`; `+(diff+RNG(0,2))*10`; floor 50). Human-only differences
  are gating: cargo pick via `CHOICE 0x15a0` instead of max-throttle
  auto-pick, then `CHOICE 0x15a9` Accept(1)/Haggle(2)/Refuse(3) loop with
...
- `+0x40-0x43` ("`treaty_timer[4]`"): **resolved 2026-08-24 (round 2) — this
  ...
  **Both closed 2026-08-27 (static)**:
  - `FUN_1000_89a4` → `FUN_0000_9810` = canonical `FUN_15eb_3960(nation,
    bit)` (`address_mapping.csv:323`): `nation>3 → 0`, else tests bit
    `bit` of the `founding_fathers[4]` bitmap at nation`+7` (`-0x77f1`).
    It is the **founding-father-owned test**; `0x13` = 19 =
    `FF_BENJAMIN_FRANKLIN` (`founding_fathers.h:81`) — halving the war
    cooldown is Franklin's "Europeans more amenable to peace" effect.
    Cross-check: the same accessor with `9` (La Salle) gates the free
    Stockade in `FUN_15eb_1030` (`viceroy_unpacked.c:11312`).
  - `6d8e`'s `&8` bit on the `-0x77c4` relation byte: set only at
    `153e`'s common tail (`:98415`) when `local_b0` (`:97686`) = pair
    already `MET` (`0x40`) and (peace kept or target nation weaker by
    `-0x6be4` strength). So bit `0x08` = **"amicable-negotiation latch"**
    (talks ended without war). `6d8e` consumes it: when the pair's
    cooldown reads 0 and the latch is set, 1-in-3 per turn → `rel &=
    0xb7` (clear `0x08|0x40`), `rel |= 0x01` (WAR): a random relapse
    into war after a truce cools off. Only other reader: the foreign-
    affairs report string list (`:101856`, string `0x42`). Linux
    `AI_DIPLO_TREASURE_STRONGER` still squats on `0x08` as a stand-in —
    `ai_diplo.h` comment updated, no code change.

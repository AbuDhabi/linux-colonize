...
Continent flood-fill IDs (`FUN_67bf_0000`) are not written to layer2 in gen v1 (shipped AMER2 leaves layer2 zero); diagonal land cleanup (2×2 masks 6/9) is ported. RNG is exact DOS `FUN_1d1d_0e04` / `FUN_19ef_0032` (`src/core/dos_rng.c`): NEW WORLD draws customize axes (`range(0,3)`) then reseeds before `map_generate`. Tribe placement restores the **post-axes** LCG (seed + 5 axis draws), then `range(1,8)` / cargo / `FUN_6a09` — it does **not** continue the post-mapgen stream. Land mask, latitude/climate paint, forest wander, rivers, and arctic/HS tail bit-match seed 100 terrain. `FUN_6a09` capitals, satellites, and Braves match SEED100.SAV (34 tribes / 46 units). Golden fidelity: `smoke_mapgen_seed100` vs `test-saves-mapgen/SEED100.SAV` (no seed-special runtime path).

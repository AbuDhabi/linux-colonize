...
| Function | Warning |
|----------|---------|
| `FUN_4d56_417e` | ~~`Removing unreachable block (ram,0x0005180a)`~~ **fixed 2026-08-13** — clean 933-byte function recovered, self-contained in `OVL13_L0000`, ends right where `4528` begins. **Identified same day (task #5) — this is "Incite Indians" (WARPATH)**, the Missionary `@ACTIONS` order that bribes a tribe to attack a rival Euro nation (`param_3`=inciting nation charged, a picked/fixed second nation=attack target, price relation-scaled). Confirmed against `GAME.TXT`'s `@INDIANWARPATH`/`@INDIANWARPATH2` text (user-identified from the structural writeup). Closes a gap flagged "incite/WARPATH gold PARKED" in 4 other places in this project. Caller still not found and 2 of 4 price terms are unnamed byte tables — not yet wired into Linux (a real new feature, not a small edit). Full writeup: `indian_incite_417e.md` |

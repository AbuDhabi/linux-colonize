...
Continent flood-fill IDs (`FUN_67bf_0000`) are not written to layer2 in gen v1 (shipped AMER2 leaves layer2 zero); diagonal land cleanup (2×2 masks 6/9) is ported. RNG is exact DOS `FUN_1d1d_0e04` / `FUN_19ef_0032` (`src/core/dos_rng.c`): NEW WORLD draws customize axes then reseeds before `map_generate`, then continues into tribe place. Land-mask generation (blobs / budget / 15 extras) bit-matches seed 100. Climate humidity walk is ported from `FUN_684c_08c0`. Forest wander now uses the DOS type/hill/mountain switch (`LAB_684c_106a` / `11c0`) plus the whole-map forest-bit pass (`0x1286`; `FUN_281f_0d12` returns 0 at mapgen). Rivers (`FUN_684c_04a6`) are ported in structure (backup/restore, coast-seeking walks, mountain thicken, neighbour forest sprinkle) but still overproduce vs golden (~69 land river tiles vs 47), so arctic-stomp RNG and full tile paint are not yet golden. Tribe placement still approximate. Golden fidelity target: `smoke_mapgen_seed100` vs `test-saves-mapgen/SEED100.SAV` (no seed-special runtime path).

...
- Colony screen bring-up: `WOODPANL.PIK` + `PARCH.SS` (full buildings section) +
  ...
  surroundings with PHYS0; economy UI phases 1–4 + 6 (workplace assign, construction **3**/**B** buy,
...
Continent flood-fill IDs (`FUN_67bf_0000`) are not written to layer2 in gen v1 (shipped AMER2 leaves layer2 zero); diagonal land cleanup (2×2 masks 6/9) is ported. RNG is exact DOS `FUN_1d1d_0e04` / `FUN_19ef_0032` (`src/core/dos_rng.c`): NEW WORLD draws customize axes then reseeds before `map_generate`, then continues into tribe place. Land-mask generation (blobs / budget / 15 extras) bit-matches seed 100. Latitude paint uses DOS `NEG` (not abs) when the first distance probe is ≤0. Climate humidity walk is ported from `FUN_684c_08c0`. Forest wander uses the DOS type/hill/mountain switch (`LAB_684c_106a` / `11c0`); forest-bit pass (`0x1286`) forests `mask==1` tiles and coastal `mask!=1` tiles via `FUN_15eb_00a2` (water-neighbour probe). Seed-100 post-gen forests are ~359 vs golden 360; hills ~69 vs 70. Rivers (`FUN_684c_04a6`) use terrain→85c0 snapshot backup and thicken from the walker tile; land rivers ~54 vs golden 47 (still desyncs arctic stomps). Arctic/HS tail ports east sea-lane, land-neighbour ocean punch, HS-fringe trim, polar fringe RNG, final HS outlines, and bit4-forest demotion — y=2 HS matches golden `(0,1,56,57)`; y=1 arctic positions still wrong until river RNG matches. Tribe placement still approximate. Golden fidelity target: `smoke_mapgen_seed100` vs `test-saves-mapgen/SEED100.SAV` (no seed-special runtime path).

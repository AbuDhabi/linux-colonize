...
typedef struct UiDragSession {
  UiDragKind kind;
  int index;   /* cargo type, hold slot, colonist idx, ship idx, or map tile x */
  int unit_id; /* outside unit / transport / selected unit */
  int amount;  /* e.g. 100 for warehouse / market loads, or map tile y */
  bool cursor_ok;
  int cursor_w;
  int cursor_h;
  int hotspot_x; /* tip for map goto; center for cargo/unit icons */
  int hotspot_y;

 ...
 *  - Cheat menu, trade-route wizard, cheat-list & save/load popups (~line 910)
 *  - Map sprite blitting & zoom helpers (~line 2066)
 *  - Save/load slot IO, reports/score/pedia menus (~line 2280)
 *  - Europe screen rendering (~line 3007)
 *  - Colony screen render, asset/palette loading, game_create/destroy (~line 4117)
 *  - Unit selection & movement dispatch (~line 5172)
 *  - Colony UI: enter colony, drag & drop, job/building assignment (~line 6184)
 *  - Europe voyages, colony roles, dock orders (~line 7175)
 *  - End-of-turn flow, trade-route servicing, map menu actions (~line 7647)
 *  - Woodcuts/intro/popup queue servicing & main per-frame update (~line 9800)
 *  - Begin menu, full-screen render & public accessor API (~line 12945)
 *
 ...
 */
static bool game_screen_owns_display(const ColonizeGameState* game) {
...
static const char* const k_cheat_create_main_types[] = {
  ...
  NULL, NULL, NULL, NULL /* rows 9..12: brave ladder, see below */,
  NULL /* Foreign */
};

/* Rows 9..12 are the Indian arms/mounts ladder — @UNIT rows 19..22, sourced
 * from the live NAMES.TXT catalog rather than a fourth private copy of the
 * spelling (audit theme D). They double as units_find_type keys, which is why
 * reports_brave_ladder_name keeps the @UNIT spelling. */
#define GAME_CHEAT_CREATE_BRAVE_FIRST 9
static const char* game_cheat_create_type_name(int id) {
  if (id >= GAME_CHEAT_CREATE_BRAVE_FIRST && id <= GAME_CHEAT_CREATE_BRAVE_FIRST + 3) {
    return reports_brave_ladder_name(id - GAME_CHEAT_CREATE_BRAVE_FIRST);
  }
  if (id < 0 || id >= (int)(sizeof(k_cheat_create_main_types) / sizeof(k_cheat_create_main_types[0]))) {
    return NULL;
  }
  return k_cheat_create_main_types[id];
}
...
static void game_apply_cheat_create_unit(ColonizeGameState* game, int id) {
  ...
  const char* type_name = game_cheat_create_type_name(id);
  if (!type_name) {
    return;
  }
...
void game_trade_open_cargo_picker(ColonizeGameState* game, int stop_i, bool is_load) {
  ...
  for (int c = 0; c < 16 && count < CHEAT_LIST_MAX_OPTIONS; ++c) {
    if (cargo_sec && c < cargo_sec->line_count) {
      ...
      snprintf(bufs[count], sizeof(bufs[count]), "Cargo %d", c);
    }
    labels[count] = bufs[count];
    ids[count] = c;
    count++;
...
static void game_trade_wizard_after_dest1(ColonizeGameState* game, int dest1) {
  ...
  popup_msg_fill(
    ...
    "Is this a {sea} trade route or a {land} trade route?", body, sizeof(body)
  );
  char label_buf[2][POPUP_MSG_CHOICE_LEN];
  const char* labels[2];
  (void)popup_msg_section_labels(
    &game->messages, "TRADETYPE", NULL, "Sea route", "Land route", label_buf, labels
  );
...
static void game_show_save_load_popup(
  ...
  char fallback[160];
  if (noun) {
    snprintf(fallback, sizeof(fallback), fallback_fmt, noun, filename);
  } else {
    snprintf(fallback, sizeof(fallback), fallback_fmt, filename);
  }
  char body[AI_POPUP_BODY_LEN];
  popup_msg_fill(&game->messages, section, &tok, fallback, body, sizeof(body));
...
void game_apply_save_load_result(ColonizeGameState* game) {
  ...
  if (mode == SAVE_LOAD_MODE_SAVE) {
    ...
    if (!game_save_col1_slot(game, slot, err, sizeof(err))) {
      ...
      game_show_save_load_popup(game, "SAVEERROR", "Error saving game %s.", filename, NULL);
      return;
    }
    snprintf(
      game->status,
    ...
    game_show_save_load_popup(game, "SAVEGOOD", "%s saved as %s.", filename, "Game");
    return;
  }
  ...
  if (!game_load_col1_slot(game, slot, err, sizeof(err))) {
    ...
    const char* fallback_fmt = "Error loading game %s.";
    if (strstr(err, "obsolete") != NULL) {
      section = "LOADOLD";
      fallback_fmt = "%s is an obsolete save file.";
    } else if (strstr(err, "does not match") != NULL) {
      section = "LOADSIZE";
      fallback_fmt = "%s does not match the current map size.  It cannot be loaded at this time.";
    } else if (strstr(err, "not a valid") != NULL) {
      section = "LOADNOT";
      fallback_fmt = "%s is not a valid save file.";
    }
    game_show_save_load_popup(game, section, fallback_fmt, filename, NULL);
    return;
  }
  snprintf(
    game->status,
  ...
  game_show_save_load_popup(game, "LOADGOOD", "Loaded %s successfully.", filename, NULL);
}
...
static void game_enqueue_war_scored_choice(ColonizeGameState* game) {
  ...
  const char* labels[2];
  (void)popup_msg_section_labels(
    &game->messages, "SCORED", NULL, "That's all.", "Keep playing anyway.", label_buf, labels
  );
  const int ids[2] = {1, 2};
  ...
  if (units_is_sea(&game->units, sid)) {
    ...
    if (dest_water &&
      ...
      if (game->europe_ok) {
        ...
        const char* labels[2];
        (void)popup_msg_section_labels(
          &game->messages, "SAILHOME", NULL, "Yes, steady as she goes.",
          "No, let us remain in these waters.", label_buf, labels
        );
        const int ids[2] = {1, 0};
    ...
    if (dest_land && !dest_water) {
      ...
      }
      {
        ...
        const char* labels[2];
        (void)popup_msg_section_labels(
          &game->messages, landfall_section, NULL, "Stay With Ships", "Make Landfall", label_buf,
          labels
        );
        const int ids[] = {0, 1};
        if (!ai_popup_enqueue_choice_ctx(
 ...
 */
static void game_colony_commit_job(ColonizeGameState* game, ColonizeColony* colony, int job) {
  ColonyScreenView* csv = &game->colony_screen;
  const int ci = game_colony_selected_colonist(game);
    ...
    if (hit.kind == COLONY_HIT_TRANSPORT && hit.index >= 0 &&
      ...
      if (csv->transport_unit_id >= 0 && game->units_ok && shift) {
        ...
        if (su && hold >= 0 && hold < COLONIZE_UNIT_CARGO_MAX &&
          ...
          tok.has_number0 = true;
          popup_msg_fill(
            &game->messages, "HOWMUCH3", &tok, "How much should be moved?", prompt,
            sizeof(prompt)
          );
 ...
 */
static int game_voyage_ship_count(const ColonizeGameState* game) {
  const int hn = game->human_nation;
...
static int game_voyage_turns_for(ColonizeGameState* game, int ships) {
  const int hn = game->human_nation;
 ...
 */
static void game_order_board(ColonizeGameState* game) {
  const int sid = game->units.selected_id;
...
static void game_order_unload(ColonizeGameState* game) {
  const int sid = game->units.selected_id;
 ...
 * lane path shares.
 */
static void game_order_return_europe(ColonizeGameState* game) {
  const int sid = game->units.selected_id;
  const ColonizeUnit* ship = units_get_const(&game->units, sid);
  if (!ship || !units_is_sea(&game->units, sid)) {
...
static void game_open_goto_port_picker(ColonizeGameState* game, int uid) {
  ...
  memset(&tok, 0, sizeof(tok));
  popup_msg_fill(
    &game->messages, "SAILPORT", &tok, "Select a port to sail to:", prompt, sizeof(prompt)
  );
    ...
    case MAP_MENU_ACTION_TRADE_CREATE: {
      ...
      if (game_trade_route_count(game) >= (int)COLONIZE_COL1_TRADE_ROUTE_COUNT) {
        ...
        popup_msg_fill(
          ...
          "Only {12} trade routes can be defined, Your Excellency.\n"

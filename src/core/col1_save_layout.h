#ifndef COLONIZE_COL1_SAVE_LAYOUT_H
#define COLONIZE_COL1_SAVE_LAYOUT_H

/*
 * On-disk COLONY##.SAV layout: size constants, #pragma pack(1) wire structs
 * with their DS:0x offset comments, and the bit-level accessors that read
 * raw layout fields directly (col1_tribe_attitude*, col1_trade_nibble_*).
 * Split out of col1_save.h (2026-09-16) so the load/save/query API there
 * is not buried under ~1000 lines of wire-format detail. See col1_save.h
 * for the assembled ColonizeCol1Save struct and the API that uses this.
 */
...
typedef struct ColonizeCol1TribeState {
  ...
  uint8_t learned : 1; /* 0x02 — thunk_FUN_1000_a618 taught one colonist */
  uint8_t capital : 1; /* 0x04 */
  uint8_t scouted : 1; /* 0x08 — thunk_FUN_1000_a60c chief already spoke */
  uint8_t tribute_paid : 1; /* 0x10 — thunk_FUN_1000_a5f4 @EXTORTSTUFF once */
  uint8_t unused09 : 3;
...
#define COL1_INDIAN_ATTACK_CONFIRMED_BIT 0x04u /* FUN_465b_0000 @WHACKINDIANS answered Yes once;
                                                  FUN_4cc6_00f2 clears when alarm cools */

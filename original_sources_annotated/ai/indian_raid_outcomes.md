...
9. **Scout hostility** (`359c`-shaped) — alarm ≥90 + Scout name adjacent to Brave:
   ...
   **"Scout warned away from village."**; **despawn only if** no free tile
   ...
   tile exists stays **PARKED** (Linux never kills if displace succeeds).
...
10. **PARKED** — deep `FUN_4d56_2820` (~1.4k; thunk `2a1f_044c`) meet/raid
   ...
   full `4528` settlement body; ship harbor deep; full DOS dialog chrome;
   `@RAIDBURN` non-Town-Hall **built** building loot when stock empty (no safe
   `colonies_*` destroy API for workplace colonists). Status-line chrome for
   raid/warn is **thinned**. Cite: `indian_contact.md` PORT DEBT;
...
| `@RAIDBURN` | BURN | Building-in-production clear / lumber stub; non-Town-Hall built-building loot **PARKED** (no safe destroy API) |
